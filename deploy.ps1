...
# Añadir MercadoPago a envVars si está configurado
if ($env:MERCADOPAGO_ACCESS_TOKEN) {
    $envVars = "$envVars,MERCADOPAGO_ACCESS_TOKEN=$($env:MERCADOPAGO_ACCESS_TOKEN)"
...
Write-Host ""
Write-Host "Ejecutando migraciones (Cloud SQL Proxy debe estar corriendo)..." -ForegroundColor Yellow
& "$PSScriptRoot\migrate.ps1" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "Migración omitida o fallida. Si es la primera vez, ejecuta: .\migrate.ps1" -ForegroundColor Gray
}
Write-Host ""
